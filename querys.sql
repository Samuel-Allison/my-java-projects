...
where CUSTOMER.CUS_FNAME = 'Alfred' and CUSTOMER.CUS_LNAME = 'Ramas'
;
...
where PRODUCT.P_DESCRIPT like '%saw%'
;
...
-- 5. Use ALTER commands to add a column called ISO9000 to Vendor. This column should hold a single character and should be 'N' by default and should never be null. 
ALTER TABLE VENDOR
add IOS9000 char(1) default 'N' not null
;
...
left join invoice
on invoice.cus_code = customer.cus_code
where invoice.inv_number is NULL -- WHERE is run after all JOIN ON (final filter)
order by customer.cus_lname, customer.cus_fname
;
